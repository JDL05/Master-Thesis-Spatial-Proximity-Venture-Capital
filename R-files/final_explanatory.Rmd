...
### Success by Year
```{r Success by Year}
warm_cols <- c("No Success" = "#808080", "Success" = "#9ecac1")
...
vars <- tribble(
  ...
  "Outcome Variable", "Years to Exit", "Years from startup incorporation to exit.",
  ...
  "Fundamental Variables", "Startups Within 50km", "Number of Startups inside a 50km radius; accumulated over years.",
  "Fundamental Variables", "VCs Within 50km", "Number of Investors inside a 50km radius.",
  "Fundamental Variables", "Share Successful Startups in Environment", "Share of Successful Startups in a 10 km radius.",
  "Fundamental Variables", "Top University Count", "Number of Top Universities within a 20km radius. A Top University is ranked at position 200 or better.",
    "Fundamental Variables", "Top University Ecosystem Binary", "1 if company is within a 20km radius to a Top University. A Top University is ranked at position 200 or better.",
  ...
  "Treatment Variables", "Minimum Seed VC Distance", "Minimum distance (km) between seed investors and HQ.",
  "Treatment Variables", "Maximum Seed VC Distance", "Maximum distance (km) between seed investors and HQ.",
  "Treatment Variables", "Avg Seed Investor Pairwise Distance", "Average distance between the seed investors.", 
  "Treatment Variables", "Minimum Seed Investor Pairwise Distance", "Minimum distance between the seed investors.", 
  "Treatment Variables", "Maximum Seed Investor Pairwise Distance", "Maximum distance between the seed investors.", 
  ...
  "Treatment Variables", "Hub VC in Seed Binary", "1 if a hub VC invested in seed; else 0.", 
  ...
  "Treatment Variables",  "Corporate VC Lead in Seed Binary",       "1 if a corporate venture arm led the seed round; else 0.",
    "Treatment Variables", "Hub Lead VC in Seed Binary", "1 if a hub VC led the seed round; else 0.",
  ...
  "Control Variables", "GDP Growth (%) in Funding Year", "European GDP Growth in the startup's year of funding."
)
...
vars <- c(
  ...
  "Maximum Seed VC Distance",
  "Avg Seed Investor Pairwise Distance",
  "Min Seed Investor Pairwise Distance",
  "Max Seed Investor Pairwise Distance",
...
vars_special <- c("Years to Exit", "Exit Binary", "Funding Success")
vars_main <- setdiff(vars, vars_special)

df_long_full <- companies_seed %>%
  mutate(SuccessGroup = ifelse(Success == 1, "Successful", "Non-successful")) %>%
  select(SuccessGroup, all_of(vars_main)) %>%
  pivot_longer(-SuccessGroup, names_to = "Variable", values_to = "Value")

df_long_full_all <- companies_seed %>%
  mutate(SuccessGroup = ifelse(Success == 1, "Successful", "Non-successful")) %>%
  select(SuccessGroup, all_of(vars)) %>%
  pivot_longer(-SuccessGroup, names_to = "Variable", values_to = "Value")

# Add filtered 'Years to Exit' for successful startups only
df_long_exit <- companies_seed %>%
  filter(Success == 1 & !is.na(`Years to Exit`)) %>%
  mutate(SuccessGroup = "Successful") %>%
  select(SuccessGroup, `Years to Exit`) %>%
  pivot_longer(-SuccessGroup, names_to = "Variable", values_to = "Value")

df_long_exitbin <- companies_seed %>%
  filter(`Exit Binary` == 1) %>%
  mutate(SuccessGroup = ifelse(Success == 1, "Successful", "Non-successful")) %>%
  select(SuccessGroup, `Exit Binary`) %>%
  pivot_longer(-SuccessGroup, names_to = "Variable", values_to = "Value")

df_long_funding <- companies_seed %>%
  filter(`Funding Success` == 1) %>%
  mutate(SuccessGroup = ifelse(Success == 1, "Successful", "Non-successful")) %>%
  select(SuccessGroup, `Funding Success`) %>%
  pivot_longer(-SuccessGroup, names_to = "Variable", values_to = "Value")

# df_long_full with vars_main
df_long_main <- companies_seed %>%
  mutate(SuccessGroup = ifelse(Success == 1, "Successful", "Non-successful")) %>%
  select(SuccessGroup, all_of(vars_main)) %>%
  pivot_longer(-SuccessGroup, names_to="Variable", values_to="Value")

# Special vars with filters
df_long_exit <- companies_seed %>%
  filter(Success == 1 & !is.na(`Years to Exit`)) %>%
  mutate(SuccessGroup = "Successful") %>%
  select(SuccessGroup, `Years to Exit`) %>%
...
desc_overall_full <- df_long_full_all %>%
  group_by(Variable) %>%
  summarise(
...
new_labels <- c(
  ...
  "Min Seed VC Distance",
  "Max Seed VC Distance",
  "Avg Seed Investor Pairwise Distance",
  "Min Seed Investor Pairwise Distance",
  "Max Seed Investor Pairwise Distance",
